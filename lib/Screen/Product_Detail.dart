...
class StateItem extends State<ProductDetail> with TickerProviderStateMixin {
  ...
  Future<void> getMostFavPro() async {
    debugPrint("proIds len***${proIds1.length}");
      ...
      if (_isNetworkAvail) {
        try {
          ...
          apiBaseHelper.postAPICall(getProductFaqsApi, parameter).then(
              (getdata) {
            debugPrint("getdata***faqs****$getdata");
            bool error = getdata["error"];
            String? msg = getdata["message"];
            if (!error) {
  ...
  Future<void> getShare() async {
    ...
    Future.delayed(Duration.zero, () {
      shareLink =
          // "\n$appName\nGet fresh, healthy Exotic & Indian Vegetables, fruits delivered to your doorstep!!!!\n\nVisit our website: www.greentopli.com\n\nAndroid App:\n$androidLink$packageName\n\nFor more details call: 9172988666";

                        ...
                        if (_isProgress == false) {
                          ...
                          showDialog(
                              ...
                              builder: (ctx) => StatefulBuilder(
                                    builder: (cont, setState) => AlertDialog(
                                      title: Text(
                                        model.name!,
                                        style: Theme.of(context)
                                            .textTheme
                                                                        ...
                                                                        if (selectedId.length == sinId.length &&
                                                                          ...
                                                                          if (model.stockType == '0' ||
                                                                            ...
                                                                            _oldSelVarient =
                                                                              ...
                                                                              '2') {
                                                                            if (model.prVarientList![varSelected!].availability ==
                                                                                '1') {
                                                                              available = true;
                                                                              outOfStock = false;
                                                                              _oldSelVarient = varSelected!;
        ...
        try {
          ...
          apiBaseHelper.postAPICall(manageCartApi, parameter).then((getdata) {
            ...
            if (!error) {
              ...
              setSnackBarCustom(
                  "${model.name} is added to the cart successfully", context);
            } else {
        ...
        if (from == 1) {
          ...
          if (int.parse(MAX_ITEMS!) > cartCount) {
            ...
            context.read<CartProvider>().addCartItem(SectionModel(
                  qty: qty,
                  productList: prList,
                  varientId: model.prVarientList![_oldSelVarient].id!,
                  id: model.id,
                ));
            db.insertCart(model.id!, model.prVarientList![_oldSelVarient].id!,
                qty, context);
          } else {
            setSnackbar(
          ...
          if (int.parse(qty) > int.parse(model.itemsCounter!.last)) {
            ...
            context.read<CartProvider>().updateCartItem(model.id!, qty,
                _oldSelVarient, model.prVarientList![_oldSelVarient].id!);
            db.updateCart(
                model.id!, model.prVarientList![_oldSelVarient].id!, qty);
          }
        }
        setState(() {
          _isProgress = false;
